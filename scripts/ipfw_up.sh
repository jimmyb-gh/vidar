...
#
# /usr/sbin/daemon -f /bin/sh -c 'sleep 10; ${VIDAR_SCRIPTS}/ipfw_up.sh'
#
# Note that this script is called from ${VIDAR_SCRIPTS}/vidar_start_postgres.sh
...
echo "ipfw_up.sh kldload ipfw has been run."

echo "Creating GOOD table whitelist."
# GOOD table 
...
#
echo "Creating BAD table blacklist with one bogon."
# Set up for BAD and add a bogon.
ipfw table BAD create type addr missing
# Use the documentation address prefix
ipfw table BAD add 2001:db8:9999:9999:9999:9999:9999:9999
ipfw table BAD list
...
# Should now be secure with GOOD and BAD tables and open operation.
